...
<script>
    ...
    export default {
        ...
        methods: {
            loadData() {
                this.loaded = false;
                axios.get("/api/v1/user-management/list",{
                    params: {
            ...
            enableUser(id) {
                axios.patch("/api/v1/user-management/" + id + "/enable")
                    .then(() => {
                        this.$notify({
                            group: 'success',
                            title: this.$options.filters.message('common.success.header'),
                            text: this.$options.filters.message('user.enable.success')
                        });
                        this.loadData();
                    }).catch(() => {
                })
            ...
            disableUser(id) {
                axios.patch("/api/v1/user-management/" + id + "/disable")
                    .then(() => {
                        this.$notify({
                            group: 'success',
                            title: this.$options.filters.message('common.success.header'),
                            text: this.$options.filters.message('user.disable.success')
                        });
                        this.loadData();
                    }).catch(() => {
                })
